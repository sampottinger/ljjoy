"""vjoy Joystick abstraction shameless lifted from fscode.

Joystick abstraction with vjoy support for Windows. Lifed from fscode and
modified slightly. Original code under BSD license.

@author: nils.meier (https://code.google.com/p/fscode/)
...
class VirtualJoystick:
    ...
    def __init__(self, joystick, virtualIndex):
        ...
        self._buttons = _vjoy.GetVJDButtonNumber(self._position.index-1)
        
        self._axis = []
        for akey, pkey in VirtualJoystick._AXIS_KEYS:
            #if _vjoy.GetVJDAxisExist(self._position.index-1, akey):
            amax = ctypes.c_long()
            amin = ctypes.c_long()
    ...
    def setAxis(self, a, value):
        ...
        self._position.__setattr__(pkey, int(value))
        self._dirty = True
...
def _init():    
    ...
    if not _vjoy: 
        try:
            ...
            print "Cannot initialize support for virtual Joysticks (%s)" % e
            _log.debug(traceback.format_exc())
